...
4. **The receiver can cryptographically verify the sender's payment intent offline** before the packet even reaches the internet (Offline Receiver Confirmation via RSA signatures).
...
### Problem 4: Offline Receiver Confidence

The merchant/receiver has no internet to verify the transaction with the bank. How do they know the payment intent is legitimate and not a fake screen?

**Solution: Local Trust via RSA Signatures.**

Before encrypting the packet, the sender signs the critical payment details (VPA, amount, nonce) using their device's private key. The receiver verifies this signature offline using the sender's public key (which would be embedded in the merchant's QR code or exchanged via Bluetooth). This gives the merchant immediate cryptographic proof of the payment *intent* before the mesh network even starts routing the packet. (Note: This proves intent, but still does not guarantee the sender has sufficient balance—that requires a TEE/Secure Enclave).
